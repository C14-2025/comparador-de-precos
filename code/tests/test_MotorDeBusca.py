...
def test_catalogo_completo_caminho_valido():
    log_saida = ""
    log_saida = MonitorDeBusca.catalogo_completo("code/tests/data/produtos.csv")
    assert len(log_saida) > 0

def test_catalogo_buscando_produto_faltante():
    log_saida = MonitorDeBusca.busca_produto(nome_produto="XXXXXXXXXX", arquivo="code/tests/data/produtos.csv")
    assert log_saida == "Nenhum Produto Com Esse Nome Encontrado!!!"

def test_catalogo_buscando_produto_existente():
    log_saida = ""
    log_saida = MonitorDeBusca.busca_produto(nome_produto="SmarTphoNe", arquivo='code/tests/data/produtos.csv')
